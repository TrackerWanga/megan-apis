...
export async function generateEphoto(effectSlug: string, texts: string[]): Promise<EphotoResult> {
  ...
  try {
    ...
    if (!pageRes.ok) {
      return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: `Effect page not found (${pageRes.status})` };
    }
    ...
    if (!tokenMatch || !buildServerMatch) {
      return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: "Could not extract form tokens" };
    }
    ...
    if (!formValueMatch) {
      return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: "Could not extract form value for image generation" };
    }
    ...
    try {
      ...
      return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: "Invalid form value data" };
    }
    ...
    if (!imageResult || !imageResult.success || !imageResult.image) {
      return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: "Image generation failed after multiple attempts" };
    }
    ...
    return { success: false, creator: "Megan APIs by Tracker Wanga | Falcon Tech", error: err.message || "Ephoto360 generation failed" };
  }
}
